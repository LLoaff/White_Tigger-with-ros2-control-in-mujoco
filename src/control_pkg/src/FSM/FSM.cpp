...
FSM::FSM(ControlComponent *_ctrlcomp):_fsm_ctrl(_ctrlcomp){
    ...
    _fsm_state_list.balance_test    = new State_BalanceTest(_ctrlcomp);
...
void FSM::run(){
    ...
    _start_time = static_cast<long long>(_fsm_ctrl->_ioros->get_simulation_time() * 1000000.0);     // 以ROS时间为准，getSystemTime函数可能会有误差
...
FSMState* FSM::GetNextState(FSMStateName fsm_state_name){
    ...
    switch (fsm_state_name)
    {
        ...
        case FSMStateName::BALANCE:
            return _fsm_state_list.balance_test;
            break;
        default:
            return _fsm_state_list.invalid;
            break;
...
FSM::~FSM(){
    ...
    delete _fsm_state_list.balance_test;
}