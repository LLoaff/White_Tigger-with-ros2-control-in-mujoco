...
Mujoco_box::Mujoco_box(ControlComponent* comp):_comp(comp){
    ...
    _pos_index = 3 * box_mocap_id;
    _quat_index = 4 * box_mocap_id;
}

void Mujoco_box::BoxUpdate(Vec3 pcom,Vec4 quat){
    if (_pos_index < 0 || _quat_index < 0) {
        std::cout<<"error in imu update"<<std::endl;
        return;
    }
    ...
    _comp->_mjdata->mocap_quat[_quat_index + 0] = quat(0);
    _comp->_mjdata->mocap_quat[_quat_index + 1] = quat(1);
    _comp->_mjdata->mocap_quat[_quat_index + 2] = quat(2);
    _comp->_mjdata->mocap_quat[_quat_index + 3] = quat(3);
