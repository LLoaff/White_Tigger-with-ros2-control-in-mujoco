...
controller_interface::return_type MitControllers::update(const rclcpp::Time & time, const rclcpp::Duration & period){
    ...
    const size_t num_joints = joint_names_.size();
    {
        ...
        for (size_t i = 0; i < num_joints; ++i) {
            double q_current = state_interfaces_[3 * i + 0].get_value(); 
            double dq_current = state_interfaces_[3 * i + 1].get_value(); 
            double tau_out = latest_command_.tau[i] + latest_command_.kp[i] * (latest_command_.q[i]-q_current) + latest_command_.kd[i] * (latest_command_.dq[i]-dq_current);
        // RCLCPP_INFO(get_node()->get_logger(), " latest_command_.tau:%3f \n ", (float)latest_command_.tau[i]);
            // std::cout<<"tau_out: "<< tau_out <<std::endl;
            command_interfaces_[i].set_value(tau_out) ;
        }
    }
    ...
    if ((time - last_pub_time).nanoseconds() >= 100000000) { // 0.1秒
        ...
        for (size_t i = 0; i < num_joints; ++i) {
            ...
            msg.q[i]  = static_cast<float>(latest_command_.q[i]);
            msg.dq[i] = static_cast<float>(latest_command_.dq[i]);
            msg.kp[i] = static_cast<float>(latest_command_.kp[i]);
            msg.kd[i] = static_cast<float>(latest_command_.kd[i]);
            msg.tau[i]= static_cast<float>(latest_command_.tau[i]);
        }
...
PLUGINLIB_EXPORT_CLASS(mit_controller::MitControllers,controller_interface::ControllerInterface)